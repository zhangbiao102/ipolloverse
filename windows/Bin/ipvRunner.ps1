...
Set-Location -Path  ..\ -PassThru  2>&1 >$null
...
function  project-start {
    ...
    if ($?) {
       ...
       Start-Process -WindowStyle hidden -FilePath  .\ipvRunner.exe -ArgumentList "-c ipvrunner.json" -RedirectStandardOutput ".\log\ipvRunner.log"
       if ($?) {
          return $true
       } else {
          return $false
